...
class PaymentSummaryFragment : Fragment(R.layout.fragment_payment_summary) {
    ...
    private fun initFirebaseAnalytics() {
        ...
        val bundle = Bundle()
        bundle.putString("show_name", "paymentSummary")
    ...
    private val onClickCallback = View.OnClickListener { view ->
        when (view) {
            binding.btnCopyNumber -> {
                ...
                context?.toast(getString(R.string.success_clip, context?.getString(R.string.bank_account_number)))
            }
            binding.btnCopyTotal -> {
                val total = args.consultationPrice

                context?.clip(total.toString())
                context?.toast(getString(R.string.success_clip, context?.getString(R.string.total_payment)))
            }
